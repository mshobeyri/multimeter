...
function extractSecret(auth: {type: string; token?: string; value?: string; password?: string}|undefined): string {
  if (!auth) {
    return '';
  }
  if (auth.type === 'bearer') {
...
/**
 * Engine-specific headers on top of API-style auth (Anthropic x-api-key, Google x-goog-api-key, …).
 */
export function probeHeadersForEngine(
    engine: string,
    auth: {type: string; token?: string; value?: string; password?: string; header?: string}|undefined|'none',
    headers: Record<string, string>,
    ): Record<string, string> {
  const h = {...headers};
  const id = normalizeEngineId(engine);
  const resolvedAuth = !auth || auth === 'none' ? undefined : auth;
  const secret = extractSecret(resolvedAuth);

  if (id === 'anthropic') {
    if (secret && !headerKey(h, 'x-api-key')) {
      h['x-api-key'] = secret;
    }
    const authHeader = headerKey(h, 'authorization');
    if (authHeader && resolvedAuth?.type === 'bearer') {
      delete h[authHeader];
    }
    if (!headerKey(h, 'anthropic-version')) {
      h['anthropic-version'] = '2023-06-01';
    }
  }

  if (id === 'google') {
    if (secret && !headerKey(h, 'x-goog-api-key')) {
  ...
  if (id === 'azure-openai') {
    if (secret && !headerKey(h, 'api-key') && !headerKey(h, 'authorization')) {
      h['api-key'] = secret;
    }
  }
