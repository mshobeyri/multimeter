...
export default class CertificatesPanel implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('multimeter.certificates') || 
          event.affectsConfiguration('multimeter.enableCertificateValidation')) {
        this.refreshWebview(webviewView);
      }
    });
  ...
  private async addClientCertificate() {
    ...
    vscode.window.showInformationMessage(`Client certificate "${name}" added successfully`);
  }
  ...
  private async toggleClientCert(id: string) {
    ...
    if (index !== -1) {
      ...
      await config.update('certificates.clients', clients, vscode.ConfigurationTarget.Workspace);
    }
  }

  private async toggleCaCert() {
    const config = vscode.workspace.getConfiguration('multimeter');
    const caCert = config.get<CaCertificate>('certificates.ca', { enabled: false, certPath: '' });
    
    caCert.enabled = !caCert.enabled;
    await config.update('certificates.ca', caCert, vscode.ConfigurationTarget.Workspace);
  }
  ...
  private async removeClientCert(id: string) {
    ...
    if (cert) {
      const confirm = await vscode.window.showWarningMessage(
        `Remove certificate "${cert.name}"?`,
        'Remove'
      );
      
      if (confirm) {
        const newClients = clients.filter(c => c.id !== id);
        await config.update('certificates.clients', newClients, vscode.ConfigurationTarget.Workspace);
        vscode.window.showInformationMessage(`Certificate "${cert.name}" removed`);
      }
    }
  }
  ...
  private async toggleSslValidation() {
    ...
    await config.update('enableCertificateValidation', !current, vscode.ConfigurationTarget.Workspace);
  }
