...
export const importsToJsfunc =
    async(imports: Record<string, string>, _visitedPaths: Set<string> = new Set(), rootPath?: string): Promise<string> => {
      const detailed = await importsToJsfuncDetailed(imports, _visitedPaths, rootPath);
      return detailed.js;
    };

export const importsToJsfuncDetailed =
    async(imports: Record<string, string>, _visitedPaths: Set<string> = new Set(), rootPath?: string): Promise<ImportGenerationResult> => {
      try {
        ...
        // Choose public function names based on file title, falling back to filename.
        ...
        const toImportObject = (importMap: Record<string, string>, baseFilePath: string|undefined): string => {
          const entries: string[] = [];
          for (const [key, requestedPathRaw] of Object.entries(importMap || {})) {
            if (!isValidJsIdentifier(key)) {
              throw new Error(`Invalid import key "${key}": must be a valid JS identifier`);
            }
            const requestedPath = String(requestedPathRaw ?? '');
            // Reuse fileImporter path resolution by building a tiny importer with this root.
            // (We only need its resolveAgainst behavior indirectly, so we resolve by calling resolveAll on a single-entry map.)
            // To keep core platform-neutral, we resolve by constructing an importer and reading from its results.
            // Note: it will load the file; but it's cached in createFileImporter.
            // For alias mapping we only need the resolved path; load is acceptable.
            // Implementation detail: resolveAll already produced these resolved paths; we find matching requestedPath.
            // Prefer direct match against resolved list for (importName, requestedPath).
            const match = resolved.find(r => r.importName === key && r.requestedPath === requestedPath);
          ...
          return `const imports = {${entries.length ? '\n  ' + entries.join(',\n  ') + '\n' : ''}};`;
        };
...
export const testToJsfunc =
  ...
  let importedFuncs = await importsToJsfunc(ctx.test.import ?? {}, visitedPaths, ctx.filePath);
  const paramsAsObj: Record<string, string> = Object.fromEntries(
