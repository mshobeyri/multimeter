...
async function run() {
  ...
  for (const name of ['multimeter.mmt', 'post_echo.mmt', 'get_json.mmt']) {
  ...
  // === Reusable helpers for the multi-file demo flow ===
  ...
  async function typeYamlContent(yamlLines) {
    ...
    for (let i = 0; i < yamlLines.length; i++) {
      ...
      if (i === 0 && line.startsWith('type: api')) {
        ...
        if (box2) {
          await moveMouse({ x: Math.round(box2.x + 50), y: Math.round(box2.y + 50) });
  ...
  async function clickReloadButton() {
    console.log('Clicking Reload in environment panel...');
    for (const frame of win.frames()) {
      try {
        const reloadBtn = frame.locator('button.button-icon', { hasText: 'Reload' });
        const count = await reloadBtn.count();
        if (count > 0) {
          const box = await reloadBtn.first().boundingBox();
          if (box && box.width > 0 && box.height > 0) {
            const target = { x: Math.round(box.x + box.width / 2), y: Math.round(box.y + box.height / 2) };
            await moveMouse(target);
            await win.mouse.click(target.x, target.y);
            console.log('Clicked Reload at', target);
            break;
          }
        }
      } catch (e) {}
  ...
  // === Phase 1: Type and configure environment (multimeter.mmt) ===
  await openExplorerFile('multimeter.mmt');
  await startRecording();

  console.log('Typing environment YAML into multimeter.mmt...');
  await typeYamlContent([
    'type: env',
    'variables:',
    '  base_url: ',
    '    local: http://localhost:8787',
    '    cloud: https://test.mmt.dev',
    '  custom_header: ',
    '    - multimeter-example',
    'presets:',
    '  server:',
  ...
  // Click Reload in the environment panel to apply variables.
  await clickReloadButton();

  // Switch base_url variable from "local" to "cloud".
  console.log('Switching base_url from local to cloud...');
  for (const frame of win.frames()) {
    try {
      const row = frame.locator('tr', {
        has: frame.locator('td.environment-table-name', { hasText: 'base_url' })
      });
      const varSelect = row.locator('select.flat-select');
      const count = await varSelect.count();
      if (count > 0) {
        const box = await varSelect.first().boundingBox();
        if (box && box.width > 0 && box.height > 0) {
          await moveMouse({ x: Math.round(box.x + box.width / 2), y: Math.round(box.y + box.height / 2) });
          await varSelect.first().selectOption('cloud');
          console.log('Selected cloud for base_url');
          break;
        }
      }
    } catch (e) {}
  ...
  // Click Reload again to apply the cloud selection.
  await clickReloadButton();
  await win.waitForTimeout(500);

  // === Phase 2: Type and send post_echo.mmt ===
  await openExplorerFile('post_echo.mmt');

  console.log('Typing API YAML into post_echo.mmt...');
  await typeYamlContent([
    'type: api',
    'title: Post echo',
    'description: Send a JSON body to the echo endpoint using an environment-based URL',
    'url: <<e:base_url>>/echo',
    'method: post',
    'format: json',
    'headers:',
    '  X-Custom: e:custom_header',
    'body:',
    '  name: Multimeter',
  ...
  // === Phase 3: Type and send get_json.mmt ===
  await openExplorerFile('get_json.mmt');

  console.log('Typing API YAML into get_json.mmt...');
  await typeYamlContent([
    'type: api',
    'title: Get sample JSON',
    'description: Fetch a sample JSON payload using an environment-based URL',
    'url: <<e:base_url>>/json',
    'method: get',
    'format: json'
  ]);
